...
const inRange = (input, rangeStart, rangeFinish, bottomInclusive = false) =>
  bottomInclusive
    ? input >= rangeStart && input <= rangeFinish
    : input > rangeStart && input < rangeFinish;
...
const evalElapsedTimeDescription = elapsedTime => {
  ...
  else if (inRange(secondsElapsed, MINUTE, HOUR))
    return `${getMinutes(secondsElapsed)} minutes ago`;
  else if (inRange(secondsElapsed, HOUR, DAY))
    return `${getHours(secondsElapsed)} hours ago`;
  else if (inRange(secondsElapsed(DAY, YEAR)))
